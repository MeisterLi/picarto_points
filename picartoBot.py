...
async def determine_animation_and_price(message, user) :
    for animation in animations:
        if animations[animation]['trigger'] in message:
            await spend_points(user, animations[animation]['price'], animations[animation]['file'], animations[animation]['coordinates'], animations[animation]['scale'], animations[animation]['random_position'], animations[animation]['random_rotation'], str(animation), animations[animation]['fade'], animations[animation]['static'])

async def spend_points(user, price, file, coordinates, scale, random_position, random_rotation, animation_name, fade, static):
    if (user_list[user] >= price):
        user_list[user] -= price
        await trigger_obs_animation(file, coordinates, scale, user, random_position, random_rotation, animation_name, fade, static)
...
async def trigger_obs_animation(file, coordinates, scale, user, random_position, random_rotation, animation_name, fade, static):
    ...
    ws = simpleobsws.WebSocketClient(url = f'ws://{host}:{port}', password = {password}, identification_parameters = parameters)
    await ws.connect() # Make the connection to obs-websocket
    await ws.wait_until_identified() # Wait for the identification handshake to complete

    request = simpleobsws.Request('CreateInput', {'sceneName' : scene_name, 'inputName' : scene_item_name, 'inputKind': 'ffmpeg_source', 'inputSettings': {'hw_decode': True, 'local_file': file, 'looping': static}})
    ...
    request = simpleobsws.Request('SetSceneItemTransform', {'sceneName' : scene_name, 'sceneItemId' : scene_item_id, 'sceneItemTransform': {
        'alignment': 5, 
        'boundsAlignment': 0, 
        'boundsHeight': 1.0, 
...
async def update_obs_scroll_text():
    ...
    for user in user_list:
        result.append(f"{user} has {user_list[user]} point{'s' if user_list[user] != 1 else ''}")
...
async def update_text_field(new_text, host, port, password):
    ...
    request = simpleobsws.Request('SetInputSettings', {'inputName' : 'Ticker', 'inputSettings' : inputSettings})
    print(request)
...
async def fade_out(scene_item):
    ...
    await ws.connect() # Make the connection to obs-websocket
    await ws.wait_until_identified() # Wait for the identification handshake to complete
    request = simpleobsws.Request('CreateSourceFilter', {'sourceName' : scene_item, 'filterName' : 'fade', 'filterKind' : 'color_filter_v2', 'filterSettings': {'opacity': 1.0}})
    await ws.call(request)
    ...
    for _ in range(num_steps):
        ...
        request = simpleobsws.Request('SetSourceFilterSettings', {'sourceName' : scene_item, 'filterName' : 'fade', 'filterSettings': {'opacity': current_value}})
        await ws.call(request)
